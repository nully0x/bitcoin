...
BOOST_AUTO_TEST_CASE(fastrandom_tests_deterministic)
{
...

    {
        ...
        BOOST_CHECK_EQUAL(FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(1h).count(), 1271170921);
        BOOST_CHECK_EQUAL(FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(1h).count(), 2803534);

        BOOST_CHECK_EQUAL(GetRand<uint64_t>(), uint64_t{10170981140880778086u});
        BOOST_CHECK_EQUAL(GetRand<int>(), int{1689082725});
        BOOST_CHECK_EQUAL(FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(1h).count(), 2464643716);
        BOOST_CHECK_EQUAL(FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(1h).count(), 2312205);

        BOOST_CHECK_EQUAL(GetRand<uint64_t>(), uint64_t{5689404004456455543u});
        BOOST_CHECK_EQUAL(GetRand<int>(), int{785839937});
        BOOST_CHECK_EQUAL(FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(1h).count(), 93558804);
        BOOST_CHECK_EQUAL(FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(1h).count(), 507022);
    }
...
BOOST_AUTO_TEST_CASE(fastrandom_tests_nondeterministic)
{
    // Check that a nondeterministic ones are not
    {
        ...
        BOOST_CHECK(FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(1h).count() != 1271170921);
        BOOST_CHECK(FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(1h).count() != 2803534);

        BOOST_CHECK(GetRand<uint64_t>() != uint64_t{10170981140880778086u});
        BOOST_CHECK(GetRand<int>() != int{1689082725});
        BOOST_CHECK(FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(1h).count() != 2464643716);
        BOOST_CHECK(FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(1h).count() != 2312205);

        BOOST_CHECK(GetRand<uint64_t>() != uint64_t{5689404004456455543u});
        BOOST_CHECK(GetRand<int>() != int{785839937});
        BOOST_CHECK(FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(1h).count() != 93558804);
        BOOST_CHECK(FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(1h).count() != 507022);
    }
