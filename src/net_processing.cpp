...
void PeerManagerImpl::ReattemptInitialBroadcast(CScheduler& scheduler)
{
    ...
    const auto delta = 10min + FastRandomContext().rand_uniform_duration<std::chrono::milliseconds>(5min);
...
void PeerManagerImpl::MaybeSendFeefilter(CNode& pto, Peer& peer, std::chrono::microseconds current_time)
{
    ...
    // until scheduled broadcast, then move the broadcast to within MAX_FEEFILTER_CHANGE_DELAY.
        ...
        peer.m_next_send_feefilter = current_time + FastRandomContext().rand_uniform_duration<std::chrono::microseconds>(MAX_FEEFILTER_CHANGE_DELAY);
    }
}
